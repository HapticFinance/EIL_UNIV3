...
rangeFactor <- sqrt(Pb / Pa)

chunk_one <- run_calc(mu, sigma, 168) 
chunk_two <- run_calc(mu, sigma,  720) 
...
Impermament loss (IL) or divergence-loss, is a phenomena experienced by LP providers on almost all types of AMM platforms, especially for non-mean reverting assets. This phenomena has been studied extensively because of its negative impact on LP providers' returns \cite{LOE} \cite{DZ}. When modelling an asset's price with a geometric brownian motion (GBM), IL can be estimated numerically by using iterative techniques, such as the Monte Carlo method. The result of this estimation is referred to as "expected impermanent loss" (EIL). Previous work concluded that the computation of EIL for Uniswap v3 positions is analytically intractable, due to the complexity of the math expressions involving the GBM \cite{GSL}. However, a recent paper provided formulas for the analytical computation of EIL for Uniswap v3 positions \cite{CV3}. We implement the ideas presented in the paper and compare the results with the results obtained from the Monte Carlo simulation.

\section{Technical details}
The integrals in the analytical computation of EIL are calculated using the "_integrate_" R function, which is based on QUADPACK routines dqags and dqagi by R. Piessens and E. deDoncker–Kapenga.
...
\newpage
 ...
 The two-dimensional integrals for the computation of "_time in the money_" are calculated using the dblquad function from the pracma library [ref]. 
