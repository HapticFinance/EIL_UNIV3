...
values <- run_sim(P, Pa, Pb, x, y, t, mu, sigma)
rangeFactor <- sqrt(Pb / Pa)

IL_v <- values[[1]]
lvr_vector <- values[[2]]
daily_volatility_vector <- values[[3]]
real_volume_vector <- values[[4]]
S <- values[[5]]
gbm_res <- values[[6]]
...
total_time_itm <- gbm_res

position_info <- cbind(  
    as.numeric(0),
    as.numeric(total_time_itm)
)

df_position_info <- data.frame(position_info)
colnames(df_position_info) <- c("Total fees", "Time (hours)")
df_position_info <-  setNames(df_position_info, c("Total fees", "Time (hours)"))
row.names(df_position_info) <- NULL
...
# Convert the matrix to Pa data frame
